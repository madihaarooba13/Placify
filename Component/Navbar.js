...
//             {/* Search Icon */}
//             <button onClick={() => setShowSearch(!showSearch)} className="hover:text-gray-200 transition hover:cursor-pointer">
//               <Search size={20} />
//             </button>
...
const Navbar = () => {
  ...
  // 🔹 Detect click outside to close sidebar & search
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (sidebarRef.current && !sidebarRef.current.contains(event.target)) setOpen(false);
  ...
  return (
    <>
      {/* 🌈 Navbar */}
      <nav className="bg-gradient-to-b from-sky-400 to-sky-600 text-white fixed w-full top-0 left-0 z-50 shadow-md">
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          
          {/* 📱 Mobile layout (2 lines) */}
          <div className="sm:hidden w-full">
            {/* 1️⃣ Top line → Centered Logo */}
            <div className="flex justify-center items-center mb-1">
              <Link href="/" className="flex items-center gap-2">
                <img src="/image.png" alt="Placify Logo" width={45} height={45} className="rounded-lg shadow-md" />
                <span className="font-bold text-lg">Placify</span>
              </Link>
            </div>

            {/* 2️⃣ Second line → Left: Welcome | Right: Search + Logout + Menu */}
            <div className="flex justify-between items-center">
              {session && (
                <div className="text-xs font-medium text-left">
                  Welcome, <span className="text-white/95">@{username.split(" ")[0]}</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-2">
                ...
                {session ? (
                  ...
                  <Link href="/login">
                    <button className="bg-white text-black text-xs px-2 py-1 rounded shadow-sm">Login</button>
                  </Link>
                )}
                <button
                  onClick={() => setOpen(!open)}
                  className="relative w-8 h-8 flex items-center justify-center rounded-full bg-white shadow cursor-pointer"
                >
                  <div className="relative w-5 h-4">
                    <span className={`absolute block h-0.5 w-5 bg-black transition-all ${open ? "rotate-45 top-2" : "top-0"}`}></span>
                    <span className={`absolute block h-0.5 w-5 bg-black top-2 transition-all ${open ? "opacity-0" : "opacity-100"}`}></span>
          ...
          <div className="hidden sm:flex items-center justify-between w-full">
            ...
            <div className="flex items-center gap-3">
              ...
              {session ? (
                <button
                  onClick={() => signOut()}
                  className="bg-white text-black px-4 py-1.5 rounded-lg text-sm shadow-md hover:bg-gray-100"
                >
                  Logout
                </button>
              ) : (
                <Link href="/login">
                  <button className="bg-white text-black px-4 py-1.5 rounded-lg text-sm shadow-md hover:bg-gray-100">
                    Login
                  </button>
                </Link>
              )}
      ...
      {showSearch && (
        <div
          ref={searchRef}
          className="fixed top-20 left-1/2 -translate-x-1/2 w-[90%] sm:w-[60%] bg-white rounded-lg shadow-xl z-50 p-3 flex items-center gap-2"
        >
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search this page..."
            className="flex-1 px-3 py-1.5 border border-gray-300 rounded-md focus:ring-2 focus:ring-sky-400 text-gray-700 text-sm"
          />
          {matches.length > 0 && (
            <span className="text-gray-500 text-xs">{currentMatch + 1}/{matches.length}</span>
          )}
      ...
      >
        ...
        {isTeacher && (
          <>
            <hr className="my-4 border-t border-gray-400/40 mx-6" />
            <div className="px-6 mt-3 mb-10">
              <h3 className="text-base font-semibold text-gray-700 mb-3">Teacher Panel 👩‍🏫</h3>
              <ul className="flex flex-col font-medium space-y-4">
                {teacherPanel.map((item, index) => (
                  <li key={index}>
                    <Link
